...
def train_or_eval_model(model, loss_f, dataloader, train_flag=False, optimizer=None, cuda_flag=False, feature_type='text', target_names=None):
    ...
    for data in dataloader:
        ...
        n_speakers = torch.zeros(qmask.size()[1], 1).type(qmask.type())
        for i in range(umask.size(0)):
            _, max_index = torch.max(qmask.transpose(0, 1)[i], dim=1)
            n_speakers[i] = torch.max(max_index+1)

        seq_lengths = [(umask[j] == 1).nonzero().tolist()[-1][0] + 1 for j in range(len(umask))]
        log_prob = model(roberta_fea, qmask, seq_lengths, train_flag, n_speakers)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_dir', type=str, default='../outputs/meld/desd', help='saved model dir')

    parser.add_argument('--load_model_state_dir', type=str, default='../outputs/meld/base_model/desd.pkl', help='load model state dir')
    ...
    hidden_size, input_size = 128, None
    ...
    model = DESD(base_model=base_model,
